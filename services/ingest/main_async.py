...
WEB_DIR.mkdir(parents=True, exist_ok=True)
    ...
    await postgres_db.init_postgres()
...
def validate_file(file: UploadFile):
    ...
    if content_type:
        ...
        # Для Excel файлов с макросами может быть разный MIME type
            ...
            # Карта дополнительных разрешенных MIME-типов для конкретных расширений
            EXTRA_MIME_MAP = {
                ".docx": [
                    "application/msword",  # Старые версии Word
    ...
    if not content_type:
        ...
        if is_allowed:
            logger.info(f"✅ [VALIDATE] MIME type {content_type} разрешен для {file_ext}")
        else:
